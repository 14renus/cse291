...
def main(filenames_by_type,data_dir,train_type):
    ### K FOLD
    if config['verbose']:
        print('Fold {}'.format(test_type))

    ### GET DATA ###
    inputs, targets = get_train_data(filenames_by_type,train_type, BATCH_SIZE=config['batch_size'],data_dir=data_dir)
        ...
        for bs in [512,128, 256]:
            for wd in [0, 1e-5, 2e-5, 1e-4]:
                for tr in [1.0, 0.7, 0.5]:
                    for idrop in [0.5, 0.0,0.2,0.4,0.6]:
                        for hdrop in [0.5, 0.0,0.2,0.4,0.6]:
...
if __name__== "__main__":
    ...
    main(filenames_by_type,data_dir,test_type)
